/*
 * Nexell Swallow processors Timer registers definition.
 *
...
#define     NEXELL_TIMER_TIMERS_NUM      4
#define     NEXELL_TIMER_REG_MEM_SIZE    0x10000
...
typedef struct {
	...
	struct NexellTIMERState *parent;

} NexellTIMER;

#define TYPE_NEXELL_TIMER "nexell.timer"
#define NEXELL_TIMER(obj) \
    OBJECT_CHECK(NexellTIMERState, (obj), TYPE_NEXELL_TIMER)

typedef struct NexellTIMERState {
	SysBusDevice parent_obj;

	MemoryRegion iomem;

	NexellTIMER timer[NEXELL_TIMER_TIMERS_NUM];

} NexellTIMERState;

enum {
	NEXELL_TIMEBASE_FREQ = 10000000
};
